...
const ModalEditMenu: React.FC<MealPrepModalProps> = ({ isOpen, onClose, onSubmit, existingMenu }) => {
  const [ingredientList] = useState<FormData[]>([
  ...
  const [gridData, setGridData] = useState<GridRow[]>(
    existingMenu?.ingredients.map((ingredient) => ({
  ...
  const handleSubmit = () => {
    ...
    axios
      .put(`http://localhost:8080/api/menu/updateMenu/${oldname}`, formData, {
        headers: {
  ...
  return (
    <div className="fixed inset-0 bg-[#1E1E1E] bg-opacity-30 flex justify-center items-center z-50">
      <div className="bg-[#F1FCF1] p-6 rounded-3xl shadow-lg w-[700px] h-[828px] relative">
        ...
        <div className="text-[#066426] text-3xl font-medium justify-self-center mt-5 mb-12">Edit Menu Information</div>
          ...
          <div className="w-[341px] justify-self-center flex justify-between items-center mb-4">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const selectedIndex = ingredientList.findIndex(
