...
const IngredientManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    axiosInstance.get('/api/ingredient/getIngredients').then((res) => {
  ...
  const handleNewIngredient = (newIngredient: FormData) => {
    axiosInstance
      .post('/api/ingredient/createIngredient', {
  ...
  const handleDeleteIngredient = (ingredientName: string) => {
    axiosInstance
      .delete(`/api/ingredient/deleteIngredient/${ingredientName}`)
      .then((res) => {
        console.log(res)
  ...
  return (
    <div className="flex flex-col items-center bg-[#85bdbf] min-h-screen py-8">
      ...
      <div className="w-[573px] grid grid-cols-3 gap-2 text-center font-semibold mb-2 text-[#386C5F]">
        ...
        <div className="w-[165px] h-[48px] bg-[#F3FFF9] rounded-full flex justify-center items-center">
          Price / Unit
        </div>
      ...
      <div className="overflow-y-auto max-h-[300px] rounded-lg p-2 custom-scrollbarIngredient">
        {filteredIngredients.map((ingredient, index) => (
          <div
            key={index}
            className="w-[590px] grid grid-cols-3 gap-2 text-center items-center mb-2 text-[#f0f0f0] text-lg"
          >
            <div className="w-[195px] h-[48px] flex justify-center items-center">{ingredient.ingredientName}</div>
            <div className="w-[165px] h-[48px] flex justify-center items-center justify-self-center">
              {ingredient.pricePerUnit}
            </div>
            <div className="flex justify-self-end gap-2">
              <div className="w-[112px] h-[48px] flex justify-center items-center px-2 justify-self-end">
                {ingredient.unit}
              </div>
              <div
                className="flex justify-center items-center hover:cursor-pointer"
                onClick={() => openPopUpEdit(ingredient)}
              >
      ...
      {isPopUpOpen && (
        <Modallngred isOpen={isPopUpOpen} onClose={() => setIsPopUpOpen(false)} onSubmit={handleNewIngredient} />
      )}
          ...
          onSubmit={(updatedIngredient) => {
            ...
            setIngredientList((prevList) =>
              prevList.map((ingredient) =>
                ingredient.ingredientName === selectedIngredient.ingredientName ? updatedIngredient : ingredient,
              ),
            )
            axiosInstance
              .put(`/api/ingredient/updateIngredient/${selectedIngredient.ingredientName}`, {
