    ...
    const fetchTopMenus = async () => {
      try {
        ...
        response.data.menus.forEach((menu: any) => {
          ...
          const logo = packageType === 'Basic' ? ricelogo : packageType === 'Deluxe' ? diamondlogo : crownlogo

          const image = packageType === 'Basic' ? basicfood : packageType === 'Deluxe' ? deluxefood : premiumfood
  ...
  return (
    <div className="mx-4 md:mx-40 lg:mx-40">
      ...
      <div className="flex items-center justify-center mx-20 mt-3">
        <div className="grid grid-cols-1 md:grid-cols-3 mx-10">{renderMenu(groupedMenus.Basic, 'Basic')}</div>
      </div>

      {/* Deluxe Menu */}
      <div className="text-topic font-sans font-bold text-[32px] mt-5 mx-24">Top Deluxe Menu</div>
      <div className="flex items-center justify-center mx-20 mt-3">
        <div className="grid grid-cols-1 md:grid-cols-3 mx-10">{renderMenu(groupedMenus.Deluxe, 'Deluxe')}</div>
      </div>

      {/* Premium Menu */}
      <div className="text-topic font-sans font-bold text-[32px] mt-5 mx-24">Top Premium Menu</div>
      <div className="flex items-center justify-center mx-20 mt-3">
        <div className="grid grid-cols-1 md:grid-cols-3 mx-10">{renderMenu(groupedMenus.Premium, 'Premium')}</div>
      </div>
    </div>
  )
}
