...
const ShippingSelectMonth: React.FC = () => {
  ...
  const filteredOrders = orders
    .filter((order: DailyOrderList) => {
      const orderDate = new Date(order.date)
      return new Intl.DateTimeFormat('en-US', { month: 'long' }).format(orderDate) === month
    })
    .map((order: DailyOrderList) => {
      const shippingStatus =
        shippings.find((shipping: Shipping) => shipping.tracking_number === order.tracking_number)?.status ||
        'In coming'
      ...
      <div className="flex flex-row flex-wrap gap-10">
        {filteredOrders.map((order: any, index: number) => (
          <div key={index} className="flex flex-col items-center mb-4 p-4 gap-8">
            ...
            <div className={`px-4 py-2 rounded-full ${getStatusStyles(order.status)}`}>{order.status}</div>
          </div>
        ))}
      </div>
    </div>
