    ...
    const fetchData = async () => {
      try {
        const [dailyOrderRes, shippingRes] = await Promise.all([
          axiosInstance.get('/api/dailyorderlist/getAllOrders'),
          axiosInstance.get('/api/shipping/getAllShippings'),
        ])

        const dailyOrderData: DailyOrderList[] = dailyOrderRes.data.data.map((item: DailyOrderList) => ({
        ...
        const shippingData: Shipping[] = shippingRes.data.map((item: Shipping) => ({
          tracking_number: item.tracking_number,
          status: item.status,
        }))
        ...
        dailyOrderData.forEach((order: DailyOrderList) => {
          ...
          const shippingStatus =
            shippingData.find((shipping: Shipping) => shipping.tracking_number === order.tracking_number)?.status || ''
