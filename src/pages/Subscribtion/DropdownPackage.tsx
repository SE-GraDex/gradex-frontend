...
const DropdownPackage: React.FC<DropdownPackageProps> = ({ onSubmit }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <img
          src={packageItems[selectedPackage as keyof typeof packageItems]}
          alt={selectedPackage}
      ...
      {isOpen && (
        <div id="dropdown" className="absolute z-50 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 ">
          <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
            {packages.map((item: 'Basic' | 'Deluxe' | 'Premium') => (
              <li key={item}>
                <div
                  className="flex bg-white px-4 py-2 hover:bg-white dark:hover:bg-white dark:hover:text-green-600 text-[#066426] hover:cursor-pointer"
                  onClick={() => {
