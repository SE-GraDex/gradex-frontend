...
const CalendarTest: React.FC<CalendarProps> = ({ isToggle, isMonthSelectorOpen, onDateSelect, toggle, months }) => {
  ...
  useEffect(() => {
    ...
    setCurrentYearState(currentYear) // new
  ...
  return (
    <div>
      <div
        className={`flex items-center justify-self-end space-x-4 text-4xl mb-2 font-bold  transition-colors duration-500 `}
      >
      ...
      <div className="bg-[#8FC6B2] p-4 rounded-lg shadow-2xl	">
        <div className={`grid grid-cols-7 gap-2 mt-2 text-center text-white transition-colors duration-500`}>
          {daysOfWeek.map((day) => (
            <div key={day} className="text-sm bg-[#1E1E1E] p-2 rounded-3xl font-semibold">
              {day}
            </div>
          ))}
        ...
        <div className="font-medium mt-8">Selected Date: {selectedDate.toDateString()}</div>
      </div>
    </div>
  )
}
