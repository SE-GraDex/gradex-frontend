...
const ImageSlider: React.FC<switchCardProps> = ({ Toggle }) => {
  ...
  useEffect(() => {
    const fetchMenus = async () => {
      try {
        const packageResponse = await axiosInstance.get('/api/user/getCurrentUserPackage', {
        ...
        const mappedMenus: MenuItem[] = response.data
          .map((item: any) => ({
  ...
  const filteredItems = menuItemTest.filter((item) => item.name.includes(searchTerm))
        ...
        <div className="flex overflow-hidden w-full justify-center space-x-4">
          {filteredItems
            ...
            .map((item, index) => (
              ...
              >
                <img src={packageItems[item.PackageName || 'Basic']} className="w-[20px] h-[20px] rounded-full mb-2" />
                <img src={item.image} alt={item.name} className="w-[100px] h-[100px] rounded-full" />
